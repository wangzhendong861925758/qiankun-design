...
// ---- 表演深度增强（v2.2.0）----
// 官方/社区共识：① 微表情用"可视觉化动作"描述，不用抽象情绪词（"眼眶泛红强忍泪水"优于"悲伤"）；
// ② 复杂/打斗动作按时间顺序拆解（空间层+时间层），拆成"起手蓄力→发力瞬间→力量传递→收势回防"；
// ③ 物理真实感锚定（重心转移、呼吸、肌肉发力、受力反馈）显著提升动作可信度；
// ④ 避坑：超幅剧烈动作易肢体变形，需约束在人体结构合理范围内。
// 本模块铁律：只补充"怎么演"，绝不改变用户的人物动作、台词、剧情——增强段以
// "在不改变上述剧情与动作的前提下"开头，且只命中剧本明确出现的情绪/打斗词。
const EMOTION_MAP = [
  [/愤怒|生气|恼怒|暴怒|怒火|怒视/, '眉头紧锁、咬肌收紧、鼻翼微张，呼吸加重，眼神锐利逼人'],
  [/悲伤|难过|伤心|心痛|哀伤/, '眼眶泛红、睫毛轻颤、嘴角微微下垂，喉结滚动强忍情绪，声音微微发颤'],
  [/害怕|恐惧|惊恐|畏缩/, '瞳孔微缩、身体不自觉后倾、手指收紧攥住衣物，呼吸变浅变快'],
  [/紧张|不安|忐忑|心慌/, '吞咽动作、指尖轻颤、肩膀微耸，视线快速游移不定'],
  [/害羞|脸红|羞涩|不好意思/, '脸颊泛红、微微低头、视线躲闪，指尖轻绞衣角'],
  [/惊讶|震惊|吃惊|愕然|目瞪口呆/, '眉毛高挑、瞳孔放大、嘴唇微张，身体瞬间僵住'],
  [/开心|高兴|喜悦|欣喜|雀跃/, '眼角弯起、嘴角上扬、眼中带笑意，肢体放松舒展'],
  [/失望|沮丧|低落|心灰/, '眼神黯淡、肩膀下垂、嘴角失去弧度，动作变得迟缓沉重'],
  [/坚定|决然|毅然|斩钉截铁/, '下颌收紧、目光沉稳直视前方，身体站定不动摇'],
  [/犹豫|迟疑|踌躇/, '视线游移、脚步停顿、手欲动又止'],
  [/疲惫|劳累|倦怠|精疲力尽/, '眼皮沉重、动作迟缓、肩膀下沉，呼吸绵长'],
  [/不屑|嘲讽|冷笑|讥讽|嗤之以鼻/, '嘴角单侧上扬、眼神斜视、眉毛微挑'],
  [/温柔|怜爱|心疼|慈爱/, '眼神柔和、眉眼舒展，动作轻缓小心'],
  [/期待|盼望|憧憬/, '身体微微前倾、眼睛发亮，呼吸轻微屏住'],
  [/尴尬|窘迫/, '视线飘忽、干笑两声，手足无措的小动作'],
  [/懊悔|后悔|自责/, '闭眼皱眉、摇头轻叹，双手攥紧或掩面'],
  [/得意|骄傲|自豪/, '下巴微抬、嘴角上扬带笑，步态轻快自信']
];
const FIGHT_RE = /打斗|激战|搏斗|交手|对打|厮杀|武打|过招|出拳|挥拳|冲拳|重拳|直拳|摆拳|肘击|膝撞|飞踢|侧踢|回旋踢|踢向|一脚踢|劈向|砍向|刺向|挥剑|拔剑|举刀|抡|劈砍|劈了|格挡|招架|闪避|躲开攻击|翻滚闪|后空翻|前翻|跃起|腾空|扑向|扑倒|推搡|扭打|抱摔|锁喉|擒拿/;
// 生成"表演细节"段：情绪微表情（命中剧本情绪词）+ 打斗时序化物理增强（命中打斗词）+ 通用物理锚定。
// 未命中任何情绪/打斗词时仍注入通用物理锚定（与任何剧情不冲突的纯真实感增益）。
function performanceLine(text) {
  const t = String(text || '');
  const out = [];
  for (const [re, detail] of EMOTION_MAP) if (re.test(t)) out.push(detail);
  if (FIGHT_RE.test(t)) out.push('打斗动作按「起手蓄力→发力瞬间→力量传递→收势回防」的节奏时序展开：腰胯扭转带动肩臂发力，发力瞬间肌肉绷紧、衣袖随劲风翻飞，重心随攻防自然转移，脚步移动抓地扎实；交手双方距离变化与冲击受力真实可信，受击方身体受力后仰、脚步踉跄后退；快慢节奏对比鲜明，关键招式清晰连贯，动作幅度始终保持在人体结构合理范围内，高帧率动作流畅不拖影');
  out.push('呼吸起伏随情绪与动作自然变化，身体重心随动作自然转移，手部与指尖细节真实细腻');
  return '表演细节：在不改变上述剧情、人物动作与台词的前提下，人物表演补充以下细节——' + out.join('；');
}
